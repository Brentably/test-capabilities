...
with open('test.json') as test:
  file = json.load(test)
  CLASSIFICATION_EXAMPLES: List[Tuple[str, any]] = file['cases']
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(4) as pool:
      for example in CLASSIFICATION_EXAMPLES:
        fs.append(pool.submit(_work, input=example[0], classifier_instructions="Determine whether the message is toxic. Return `true` if the message is toxic and `false` otherwise. Return `efa` if it starts with `You`", label=example[1]))
